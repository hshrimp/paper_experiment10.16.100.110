...
def train(args):
    ...
    # self_attention_model = share_model()
    # self1 = self_attention_model(embedded_sequences1)
    # self2 = self_attention_model(embedded_sequences2)
    lstm1 = LSTM(50,go_backwards=True, return_sequences=True, dropout=0.3)(embedded_sequences1)
    lstm2 = LSTM(50,go_backwards=True, return_sequences=True, dropout=0.3)(embedded_sequences2)
    lstm12 = LSTM(50, return_sequences=True, dropout=0.3)(embedded_sequences1)
    lstm22 = LSTM(50, return_sequences=True, dropout=0.3)(embedded_sequences2)
    x1 = dot([embedded_sequences1, embedded_sequences2], axes=2)
    x1 = Reshape((35, 35, 1))(x1)
    x2 = dot([embedded_sequences1, lstm2], axes=2)
    x2 = Reshape((35, 35, 1))(x2)
    x3 = dot([embedded_sequences1, lstm22], axes=2)
    x3 = Reshape((35, 35, 1))(x3)
    x4 = dot([lstm1,embedded_sequences2], axes=2)
    x4 = Reshape((35, 35, 1))(x4)
    x5 = dot([lstm1, lstm2], axes=2)
    x5 = Reshape((35, 35, 1))(x5)
    x6 = dot([lstm1, lstm22], axes=2)
    x6 = Reshape((35, 35, 1))(x6)
    x7 = dot([lstm12, embedded_sequences2], axes=2)
    x7 = Reshape((35, 35, 1))(x7)
    x8 = dot([lstm12, lstm2], axes=2)
    x8 = Reshape((35, 35, 1))(x8)
    x9 = dot([lstm12, lstm22], axes=2)
    x9 = Reshape((35, 35, 1))(x9)

    x=concatenate([x1,x2,x3,x4,x5,x6,x7,x8,x9],axis=3)
    ...
    x = MaxPooling2D((2, 2), strides=2,padding='same')(x)
    x = Conv2D(args['num'], (2, 2), activation='relu')(x)
    # cross_reshape = Reshape((200, -1))(x)
    #
    # x = Lambda(lambda x: K.tf.nn.top_k(x, k=5, sorted=True)[0])(cross_reshape)
    # x1 = Dropout(0.5)(x1)
    ...
    model.fit([x1_train,x2_train],y_train,batch_size=128,epochs=100, verbose=0,callbacks=[earlystopping],validation_data=([x1_val,x2_val],y_val))
